...
export function BookingsTab() {
    ...
    const fetchBookings = async () => {
        setLoading(true);
        try {
            console.log('Fetching bookings with params:', { pageNumber: currentPage, pageSize, orderBy: sortBy });
                                    ...
                                    {booking.hookahs.map((hookah, index) => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                <p className="font-medium text-gray-900">{hookah.tobaccoFlavor}</p>
                                                <span className="text-sm text-gray-600">
                                                    {t(`booking.strengthLevels.${hookah.strength.toLowerCase()}`)}
                                                </span>
                                            </div>
                                            {hookah.serveAfterMinutes !== null &&
                                                hookah.serveAfterMinutes !== undefined &&
                                                hookah.serveAfterMinutes > 0 && (
                                                    <p className="text-sm text-gray-600">
